...
class LoginActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.apply {
            loginButton.setOnClickListener {
                // 检查用户账户是否为空
                if (userAccountEdit.text.toString().isEmpty()) {
                    showToast(GlobalUtil.getString(R.string.user_acoount_is_empty))
                    return@setOnClickListener
                } else if (userAccountEdit.text.toString().length < 4) {
    ...
    private fun showPrivacyPolicyDialog() {
        // 检查用户是否已同意隐私策略 暂时没用
        if (getSharedPreferences(Const.Shared.PRIVACY_POLICY, MODE_PRIVATE).getBoolean(
